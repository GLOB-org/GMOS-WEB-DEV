    ...
    async getDataProduk(kategori_id) {
        ...
        if (checkLogin != null) {
            ...
            if (localStorage.getItem('TipeBisnis') != null) {
                if (decrypt(localStorage.getItem('TipeBisnis')) == 1) {
                    ...
                    if (kategori_id == 100) {
                        var get_produk = encrypt("select a.barang_id as id, a.total, c.nama, c.category_id,  b.company_id, d.kode_seller, d.nama_perusahaan, b.foto, b.flag_foto, b.kode_barang, b.price, b.price_terendah, case when price = price_terendah then 'no' else 'yes' end as negotiable, " +
                            "b.jumlah_min_beli, b.jumlah_min_nego, b.deskripsi, c.berat, e.alias as satuan, f.nominal as kurs  from (" +
                            "select barang_id, sum(qty) as total from gcm_transaction_detail where create_date >= (now() - INTERVAL '30 DAY')::DATE " +
                    ...
                    if (kategori_id == 100) {
                        ...
                        var get_produk = encrypt("select a.barang_id as id, a.total, c.nama, c.category_id,  b.company_id, d.kode_seller, d.nama_perusahaan, b.foto, b.flag_foto, b.kode_barang, b.price, b.price_terendah, case when price = price_terendah then 'no' else 'yes' end as negotiable, " +
                            ...
                            "select barang_id, sum(qty) as total from gcm_transaction_detail where create_date >= (now() - INTERVAL '30 DAY')::DATE " +
                            "group by barang_id) as a, gcm_list_barang b, gcm_master_barang c,  gcm_master_company d, gcm_master_satuan e, gcm_listing_kurs f  where a.barang_id = b.id and b.barang_id=c.id and b.company_id = d.id " +
                            "and c.satuan = e.id and c.category_id != 1 " + 
                            "and f.company_id = b.company_id and b.company_id in (" + this.state.seller + ") and b.status = 'A' and now() between f.tgl_start and f.tgl_end ORDER by a.total desc FETCH FIRST 5 ROWS only")
                        ...
                        var get_produk = encrypt(" select * from (select a.barang_id as id, a.total, c.nama, c.category_id,  b.company_id,  d.kode_seller, d.nama_perusahaan, b.foto, b.flag_foto, b.kode_barang, b.price, b.price_terendah, case when price = price_terendah then 'no' else 'yes' end as negotiable, " +
                            "b.deskripsi, b.jumlah_min_beli, b.jumlah_min_nego, c.berat, e.alias as satuan, f.nominal as kurs   from (" +
                            "select barang_id, sum(qty) as total from gcm_transaction_detail where create_date >= (now() - INTERVAL '30 DAY')::DATE " +
                            "group by barang_id) as a, gcm_list_barang b, gcm_master_barang c,  gcm_master_company d, gcm_master_satuan e, gcm_listing_kurs f  where a.barang_id = b.id and b.barang_id=c.id and b.company_id = d.id " +
                            "and c.satuan = e.id and c.category_id != 1 " +
            ...
            if (kategori_id == 100) {
                ...
                var get_produk = encrypt("select a.barang_id as id, a.total, c.nama, c.category_id,  b.company_id, d.kode_seller, d.nama_perusahaan, b.foto, b.flag_foto, b.kode_barang, b.price, b.price_terendah, b.jumlah_min_beli, b.jumlah_min_nego, b.deskripsi, c.berat, e.alias as satuan   from (" +
                    "select barang_id, sum(qty) as total from gcm_transaction_detail where create_date >= (now() - INTERVAL '30 DAY')::DATE " +
                ...
                var get_produk = encrypt("select a.barang_id as id, a.total, c.nama, c.category_id,  b.company_id, d.kode_seller, d.nama_perusahaan, b.foto, b.flag_foto, b.kode_barang, b.price, b.price_terendah, b.deskripsi, b.jumlah_min_beli, b.jumlah_min_nego, c.berat, e.alias as satuan   from (" +
                    "select barang_id, sum(qty) as total from gcm_transaction_detail where create_date >= (now() - INTERVAL '30 DAY')::DATE " +
        ...
        await Axios.post(url.select, {
            ...
            if (products_length < 5) {
                ...
                let id_get, total_get, nama_get, category_id_get, company_id_get, kode_seller_get;
                let nama_perusahaan_get, foto_get, flag_foto_get, kode_barang_get, price_get, price_terendah_get;
                let kurs_get, deskripsi_get, jumlah_min_beli_get, jumlah_min_nego_get, berat_get, satuan_get;
                ...
                for (var i = 0; i < (5 - products_length); i++) {
                    ...
                    this.setState(prevState => ({
                        products: [...prevState.products, {
                            ...
                            nama_perusahaan: nama_perusahaan_get, foto: foto_get, flag_foto: flag_foto_get,
                            kode_barang: kode_barang_get, kode_seller: kode_seller_get, price: price_get, 
                            price_terendah: price_terendah_get, deskripsi: deskripsi_get, jumlah_min_beli: jumlah_min_beli_get,
