...
export default class InfoCompanyCard extends Component{
    ...
    detailComplained = async(id)=>{
        ...
        // let queryDetailCanceled = encrypt("select a.id, a.transaction_id, c.nama, a.qty, a.qty_dipenuhi, a.harga, a.batch_number, "+
        ...
        let queryDetailComplained = encrypt("select a.id, a.transaction_id, c.nama, a.qty, a.qty_dipenuhi, a.harga, a.batch_number, "+
        "case when b.flag_foto = 'Y' then "+
        "(select concat('https://www.glob.co.id/admin/assets/images/product/', b.company_id,'/',b.kode_barang,'.png')) "+
        "else 'https://glob.co.id/admin/assets/images/no_image.png' end as foto, " +
        ...
        "d.alias as satuan, b.company_id as penjual, gmc.nama_perusahaan as nama_penjual, e.jenis_complain, f.id_transaction_ref, f.foto_transaction_ref, "+
        "e.notes_complain, f.ppn_seller, case when a.note is null or a.note = '' then '-' else a.note end as note, "+
        "case when f.tgl_permintaan_kirim is not null then to_char(f.tgl_permintaan_kirim, 'dd-MM-yyyy') else '-' end as tgl_permintaan_kirim, "+
        "case when status_payment = 'UNPAID' then case when id_list_bank is null "+
        "then 'menunggu pembayaran' else 'menunggu verifikasi' end else 'pembayaran selesai' end as payment_status "+
        "from gcm_master_satuan d, gcm_master_company gmc, "+
