    ...
    async componentDidMount() {
        ...
        await Axios.post(url.select, {
            ...
            if (decrypt(localStorage.getItem('TipeBisnis')) == 1) {
                // var get_produk = encrypt("SELECT a.nama, b.barang_id, b.id, b.kode_barang, price, price_terendah, foto, flag_foto, category_id, b.company_id, berat, b.deskripsi, b.jumlah_min_beli, b.jumlah_min_nego, c.kode_seller, c.nama_perusahaan, d.alias as satuan FROM gcm_master_satuan d ,gcm_master_company c, gcm_master_barang a inner join gcm_list_barang b on a.id=b.barang_id where a.status='A' and b.status='A' and b.company_id = c.id and d.id = a.satuan and b.company_id not in (" + set_seller + ") order by b.create_date desc, category_id asc, nama asc")
                var get_produk = encrypt("SELECT * FROM ( " +
                    ...
                    "and b.company_id not in (" + set_seller + ") and a.category_id !=5) " +
                    ...
                    "inner join gcm_list_barang b on a.id=b.barang_id where a.status='A' " +
                    "and b.status='A' and b.company_id = c.id and d.id = a.satuan " +
                    "and b.company_id not in (" + set_seller + ") and a.category_id =5 order by barang_id, price desc )) as produk order by produk.nama")
                ...
                var get_produk = encrypt("SELECT * FROM ( " +
                    ...
                    "inner join gcm_list_barang b on a.id=b.barang_id where a.status='A' " +
                    "and b.status='A' and b.company_id = c.id and d.id = a.satuan " +
                    "and b.company_id not in (" + set_seller + ") and category_id not in (1,5)) " +
                    ...
                    "inner join gcm_list_barang b on a.id=b.barang_id where a.status='A' " +
                    "and b.status='A' and b.company_id = c.id and d.id = a.satuan " +
                    "and b.company_id not in (" + set_seller + ") and category_id = 5 and b.departmen_sales = "
                    + decrypt(localStorage.getItem('TipeBisnis')) + " )) as produk order by produk.nama")

                var get_seller = encrypt("SELECT distinct b.company_id as id, c.nama_perusahaan FROM gcm_master_satuan d ,gcm_master_company c, gcm_master_barang a inner join gcm_list_barang b on a.id=b.barang_id where a.status='A' and b.status='A' and b.company_id = c.id and d.id = a.satuan and b.company_id not in (" + set_seller + ") and category_id =  " + decrypt(localStorage.getItem('TipeBisnis')))
            }
